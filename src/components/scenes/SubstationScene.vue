...
import DeviceGrid from "../views/info/DeviceGrid.vue";
import SafetyIllustrationCanvas from "../canvas/SafetyIllustrationCanvas.vue";
...
const isSafetyPanelOpen = ref<boolean>(false);
</script>
...
<template>
    ...
    <template #pin="scope">
      ...
      >
        <transition name="fade" mode="out-in">
          <svg
              style="width: 60%; height: 60%; fill: url(#decorated-fill);"
              viewBox="0 0 1024 1024" :key="cameraViewType"
          >
            <path
                v-if="cameraViewType === CameraViewType.FirstPerson"
            ...
            />
            <path
                v-else-if="cameraViewType === CameraViewType.ThirdPerson"
  ...
  <Panel side="left" top="2.5rem" :show="loadCompleted">
    <template #pin>
      <DecoratedButton
          @click="isSafetyPanelOpen = true"
          tooltip="安全演示" tooltip-placement="right-start"
      >
        <Widget
            v-if="isSafetyPanelOpen" @closed="isSafetyPanelOpen = false"
            :control-buttons="['close']"
            window-title="安全演示"
        >
          <SafetyIllustrationCanvas/>
        </Widget>
        <svg style="width: 60%; height: 60%; fill: url(#decorated-fill)" viewBox="0 0 1027 1024" v-once>
          <path
              d="M55.165 570.027C26.152-3.413 828.285-40.533 875.22 501.76a359.7 359.7 0 0 1-116.48 109.653C713.085 345.6 562.472 203.947 307.752 184.747c258.133 49.066 362.667 215.893 384 459.093a539 539 0 0 1-62.293 18.773h-18.347L597.46 558.507l-80.64-3.84-13.227 112.213a1294 1294 0 0 1-173.227-15.787l-5.12-109.226-75.52-9.814L231.38 632.32a803.8 803.8 0 0 1-176.214-62.293"/><path d="M36.392 630.613c-18.347-11.52-56.32-38.826-23.467-75.946l7.68-7.68v46.08l17.494 9.386a830.3 830.3 0 0 0 178.346 63.574h6.827v10.666l109.653 20.48v-10.24c37.547 5.974 74.667 9.814 112.214 12.374 18.773 0 37.12 2.56 55.466 2.56v10.24H616.66V700.16h22.186a426.67 426.67 0 0 0 267.947-174.933 31.57 31.57 0 0 0 5.12-30.294 853 853 0 0 1 81.067 52.907c42.666 31.573 46.933 46.933 7.253 109.227a346.45 346.45 0 0 1-143.36 128 183.9 183.9 0 0 1-113.067 18.346C475.86 786.347 204.5 737.707 36.392 630.613"
        />
        </svg>
      </DecoratedButton>
    </template>
