<template>
  <div :style="{
    ...
    <div v-if="$slots.default" :style="{
            width: calculatedWidth * 0.5 + 'px',
            height: calculatedWidth * 0.5 + 'px',
         }"
         class="icon-slot"
    >
      <slot/>
    ...
    >
      <defs>
        ...
        <radialGradient :id="uuids[9]" cx="50%" cy="97.912%" r="33.03%" fx="50%" fy="97.912%"
                        gradientTransform="matrix(0 .9344 -1.7223 0 2.186 .512)">
          <stop :offset="0" :stop-color="tinycolor(themeColor).brighten(10).setAlpha(0.4).toRgbString()"/>
          <stop :offset="1" :stop-color="tinycolor(themeColor).lighten(30).setAlpha(0).toRgbString()"/>
        </radialGradient>
        <radialGradient :id="uuids[10]" cx="50%" cy="97.912%" r="44.268%" fx="50%" fy="97.912%"
                        gradientTransform="matrix(0 .8958 -1.7223 0 2.186 .531)">
        ...
        <path :id="uuids[12]"
              d="M4 0h48.066a4 4 0 0 1 4 4v38.238a4 4 0 0 1-2.075 3.506L28.033 60 2.074 45.744A4 4 0 0 1 0 42.238V4a4 4 0 0 1 4-4"/>
        <path :id="uuids[13]"
              d="M10.885 3.934H45.18a4 4 0 0 1 4 4V36.68a4 4 0 0 1-2.006 3.468l-19.141 11-19.141-11a4 4 0 0 1-2.007-3.468V7.934a4 4 0 0 1 4-4"/>
      ...
      <g fill="none" fill-rule="evenodd">
        <g transform="translate(7 54)">
          <ellipse cx="20" cy="7" :fill="`url(#${uuids[0]})`" :stroke="`url(#${uuids[1]})`" stroke-width=".5" rx="19.75"
                   ry="6.75"/>
          <ellipse cx="21" cy="7" :fill="`url(#${uuids[2]})`" :stroke="`url(#${uuids[3]})`" stroke-width=".5" rx="10.75"
                   ry="2.75"/>
          <ellipse cx="21" cy="7" :fill="`url(#${uuids[4]})`" :filter="`url(#${uuids[11]})`" rx="3.667" ry="1"/>
        </g>
        <use :xlink:href="`#${uuids[12]}`" :fill="`url(#${uuids[5]})`"/>
        <path :fill="`url(#${uuids[9]})`" :stroke="`url(#${uuids[6]})`" stroke-linejoin="bevel"
              d="M52.066.5c.966 0 1.841.392 2.474 1.025A3.5 3.5 0 0 1 55.566 4v38.238a3.5 3.5 0 0 1-1.816 3.067L28.033 59.43 2.315 45.305A3.5 3.5 0 0 1 .5 42.238V4c0-.966.392-1.841 1.025-2.475A3.5 3.5 0 0 1 4 .5z"/>
        <use :xlink:href="`#${uuids[13]}`" :fill="`url(#${uuids[7]})`"/>
        <path :fill="`url(#${uuids[10]})`" :stroke="`url(#${uuids[8]})`" stroke-linejoin="bevel"
              d="M45.18 4.434c.967 0 1.842.392 2.475 1.026a3.5 3.5 0 0 1 1.025 2.474V36.68a3.5 3.5 0 0 1-1.756 3.034L28.033 50.571 9.14 39.713a3.5 3.5 0 0 1-1.756-3.034V7.934c0-.966.392-1.841 1.025-2.474a3.5 3.5 0 0 1 2.475-1.026Z"/>
      </g>
    </svg>
  </div>
</template>
...
const shadowColor = ref<string>(tinycolor(themeColor.value).darken(30).toString());
</script>
...
.icon-slot {
  ...
  filter: drop-shadow(0 2pt 4pt v-bind(shadowColor));
}
</style>