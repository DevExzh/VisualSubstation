<template>
  <div :style="{
    ...
    >
      <defs>
        <radialGradient :id="uuids[0]" cx="50%" cy="4.81%" r="70.812%" fx="50%" fy="4.81%"
                        :gradientTransform="`matrix(0 1 -.83015 0 .54 -.452)`">
        ...
        <radialGradient :id="uuids[1]" cx="50%" cy="100%" r="45.554%" fx="50%" fy="100%"
                        :gradientTransform="`matrix(0 1 -1.6707 0 2.17 .5)`">
          <stop offset="0%" :stop-color="tinycolor(themeColor).lighten(40).setAlpha(0.5).toRgbString()"/>
          <stop offset="100%" :stop-color="tinycolor(themeColor).toRgbString()" stop-opacity="0"/>
        </radialGradient>
        <radialGradient :id="uuids[2]" cx="50%" cy="96.134%" r="26.569%" fx="50%" fy="96.134%"
                        :gradientTransform="`matrix(0 1 -1.9949 0 2.418 .461)`">
          <stop offset="0%" stop-color="#FFF"/>
          <stop offset="100%" :stop-color="tinycolor(themeColor).toRgbString()" stop-opacity="0"/>
        </radialGradient>
        <radialGradient :id="uuids[3]" cx="50%" cy="2.728%" r="26.569%" fx="50%" fy="2.728%"
                        :gradientTransform="`matrix(0 1 -1.9949 0 .554 -.473)`">
          <stop offset="0%" stop-color="#FFF"/>
          <stop offset="100%" :stop-color="tinycolor(themeColor).toRgbString()" stop-opacity="0"/>
        </radialGradient>
        ...
        <path id="pathShape"
              d="M24.841 45.308 32.01 56.46a1 1 0 0 1-.842 1.54H16.832a1 1 0 0 1-.842-1.54l7.169-11.152a1 1 0 0 1 1.682 0"/>
      </defs>
      <g fill="none" fill-rule="evenodd">
        <g transform="matrix(1 0 0 -1 0 102)">
          <use xlink:href="#pathShape" :fill="`url(#${uuids[4]})`"/>
          <use xlink:href="#pathShape" :fill="`url(#${uuids[0]})`"/>
        </g>
        <rect width="48" height="48" :fill="`url(#${uuids[5]})`" rx="10"/>
        <use xlink:href="#rectShape" :fill="`url(#${uuids[6]})`"/>
        <rect width="39" height="39" x="4.5" y="4.5" :fill="`url(#${uuids[1]})`" :stroke="`url(#${uuids[7]})`"
              stroke-linejoin="bevel" rx="8"/>
