...
import {ServerInfo} from "../../../../ts/common/ApiTypes.ts";
import {Cpu, Tickets, Monitor, CoffeeCup, MessageBox} from "@element-plus/icons-vue";
const server = ref<ServerInfo>();
const loading = ref<boolean>(false);
onMounted(() => {
  loading.value = true;
  Api.Monitor.Server.getServer().then(response => {
          ...
          <div class="el-table el-table--enable-row-hover el-table--medium">
            <table style="width: 100%; border-spacing: 0;">
              ...
              <tr>
                ...
                <td class="el-table__cell is-leaf">
                  <div class="cell" v-if="server.mem">{{ server.mem.total }} GB</div>
                </td>
                <td class="el-table__cell is-leaf">
                  <div class="cell" v-if="server.jvm">{{ server.jvm.total }} MB</div>
                </td>
              </tr>
              <tr>
                <td class="el-table__cell is-leaf"><div class="cell">已用内存</div></td>
                <td class="el-table__cell is-leaf">
                  <div class="cell" v-if="server.mem">{{ server.mem.used}} GB</div>
                </td>
                <td class="el-table__cell is-leaf">
                  <div class="cell" v-if="server.jvm">{{ server.jvm.used}} MB</div>
                </td>
              </tr>
              <tr>
                <td class="el-table__cell is-leaf"><div class="cell">剩余内存</div></td>
                <td class="el-table__cell is-leaf">
                  <div class="cell" v-if="server.mem">{{ server.mem.free }} GB</div>
                </td>
                <td class="el-table__cell is-leaf">
                  <div class="cell" v-if="server.jvm">{{ server.jvm.free }} MB</div>
                </td>
              </tr>
              <tr>
